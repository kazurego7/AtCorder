    ...
    public class Program {
        public static void Main (string[] args) {
            ...
            foreach (var i in Enumerable.Range (1, N)) {
                var to = Enumerable.Range (0, N - 1).Where (k =>
                        a[k] == i || b[k] == i
                    ).Select (k => a[k] == i ? b[k] : a[k])
                    .ToList ();
                edges.Add (i, to);
            }
            ...
            var manyEdges = edges.OrderByDescending (keyValue => keyValue.Value.Count);
            foreach (var edge in manyEdges) {
                var fromNode =
