...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var inAB = s.Select (si => Enumerable.Range (0, si.Count () - 1).Where (i => si[i] == 'A' && si[i + 1] == 'B').Count ()).Sum ();
            var fB = s.Where (si => si.First () == 'B' && si.Last () != 'A').Count ();
            var bA = s.Where (si => si.Last () == 'A' && si.First () != 'B').Count ();
            var onlyfBbA = s.Where (si => si.First () == 'B' && si.Last () == 'A').Count ();
            var remfBbA = Abs (fB - bA);
            if (remfBbA >= onlyfBbA) {
                var ans = inAB + Min (fB, bA) + onlyfBbA;
                Print (ans);
            } else {
                // Print ($"{inAB} {fB} {bA} {remfBbA} {onlyfBbA}");
                var ans = inAB + Min (fB, bA) + remfBbA + (onlyfBbA - remfBbA) + (fB == 0 && bA == 0 ? -1 : 0);
                Print (ans);
            }
        }
    }
