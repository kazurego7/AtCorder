    ...
    public class Program {
        public static void Main (string[] args) {
            ...
            if (K < N / M) {
                ...
                foreach (var m in Enumerable.Range (0, M + 1)) {
                    dp[0, m] = 0;
                }
                foreach (var n in Enumerable.Range (1, N)) {
                    foreach (var m in Enumerable.Range (0, M + 1)) {

                        var days = Enumerable.Range (n - (K - 1), K - 1).Where (day => 0 <= day);
                        Print ($"n:{n} m:{m}");

                        if (n < m || days.IsEmpty () || m == 0) {
                            dp[n, m] = 0;
                        } else {
                            PrintRow (days);
                            PrintRow (days.Select (d => dp[d, m - 1]));
                            dp[n, m] = Max (dp[n - 1, m], days.Select (d => dp[d, m - 1] + A[n - 1]).Max ());
                        }
                    }
                }
