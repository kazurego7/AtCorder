...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var ans = go (N, K, Enumerable.Empty<int> ());
            if (ans.IsEmpty ()) {
                Print (-1);
            } else {
                PrintRow (ans);
        ...
        public static IEnumerable<int> go (int n, int k, IEnumerable<int> res) {
            if (k == 1) {
                return res.Append (n);
            } else {
                var minDiv = Enumerable.Range (2, (int) Sqrt (n) - 1).FirstOrDefault (div => n % div == 0);
                if (minDiv == 0) return Enumerable.Empty<int> ().ToList ();
                return go (n / minDiv, k - 1, res.Append (minDiv));
            }
        }
    }
