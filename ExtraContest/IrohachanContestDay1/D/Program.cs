...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var takahashiSide = X + Enumerable.Range (0, N).Where (isEven).Select (e => sortedA[e]).Sum ();
            var aokiSide = Y + Enumerable.Range (0, N).Where (isOdd).Select (o => sortedA[o]).Sum ();
            if (takahashiSide == aokiSide) {
                Print ("Draw");
            } else if (takahashiSide < aokiSide) {
                Print ("Aoki");
            } else {
                Print ("Takahashi");
            }
