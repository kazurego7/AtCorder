...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var D = ReadLongs ().OrderBy (x => x);
            var dateDistance = D.Prepend (0).MapAdjacent ((a, b) => Abs (a - b));
            // PrintRow (dateDistance);
            var dateCounts = dateDistance.Select (d => (d - 1) / A + 1);
            // PrintRow (dateCounts);
            var lastDate = (N - D.Last ()) / A;
            // Print (lastDate);
            Print (N - (dateCounts.Sum () + lastDate));
