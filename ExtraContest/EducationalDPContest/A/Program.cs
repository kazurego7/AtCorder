...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var h = To1BasedIndexing (ReadInts ());
            var init = new Dictionary<int, int> {
                    //init
                    { 1, 0 },
                    { 2, Abs (h[1] - h[2]) },
                };
            Func<Dictionary<int, int>, int, int> rec = (dp, i) =>
                Min (dp[i - 1] + Abs (h[i - 1] - h[i]), dp[i - 2] + Abs (h[i - 2] - h[i]));
            var patterns = Enumerable.Range (1 + init.Count, N - init.Count);
            var ans = DynamicProgramming (init, rec, patterns) [N];
            Print (ans);
        }

        public static Dictionary<int, T> To1BasedIndexing<T> (IEnumerable<T> source) {
