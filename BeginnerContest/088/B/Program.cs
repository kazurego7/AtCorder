...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var a = ReadInts ();
            var sorted = a.OrderByDescending (x => x).ToList ();
            var AScore = Enumerable.Range (0, N).Where (isEven).Select (i => sorted[i]).Sum ();
            var BScore = Enumerable.Range (0, N).Where (isOdd).Select (i => sorted[i]).Sum ();
            Print (AScore - BScore);
        }
    }
