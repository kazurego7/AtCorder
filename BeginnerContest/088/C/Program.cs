...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var c = ReadIntColumns (3);

            var bs = Enumerable.Range (0, 101)
                .SelectMany (b1 => Enumerable.Range (0, 101)
                    .SelectMany (b2 => Enumerable.Range (0, 101)
                        .Select (b3 => new List<int> { b1, b2, b3 })));
            var isYes = bs.Any (b => {
                var a = c.Select (ci => ci.Zip (b, (cij, bj) => cij - bj));
                return a.All (ai => ai.MapAdjacent ((a0, a1) => a0 == a1).All (isEqual => isEqual));
            });
