...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var N = NQ[0];
            var Q = NQ[1];
            var ab = ReadColumns (N - 1, 2);
            var a = ab[0].ToInts ();
            var b = ab[1].ToInts ();
            var px = ReadColumns (Q, 2);
            var p = px[0].ToInts ();
            var x = px[1].ToInts ();
            ...
            var tree = Interval (0, N).Select (_ => new List<int> ()).ToList ();
            ...
            var counter = Enumerable.Repeat (0, N).ToList ();
            foreach (var i in Interval (0, Q)) {
                counter[p[i] - 1] += x[i];
            }

            var parent = Enumerable.Repeat (NONPARENT, N).ToList ();
            parent[0] = ROOT;
            ...
            while (!que.IsEmpty ()) {
                int v = que.Dequeue ();
                foreach (var child in tree[v]) {
                    if (parent[child] != NONPARENT) continue;
                    parent[child] = v;
                    counter[child] += counter[parent[child]];
                    que.Enqueue (child);
                }
            }
