...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var isOk = Go (String.Concat (S.Reverse ()), 0);
            PrintIf (isOk, "YES", "NO");
        }

        public static bool Go (string Sr, int i) {
            if (i >= Sr.Length) {
                return true;
            } else {
                if (EqualSubs ("maerd", Sr, i, i + 5) ||
                    EqualSubs ("esare", Sr, i, i + 5)) {
                    return Go (Sr, i + 5);
                } else if (EqualSubs ("resare", Sr, i, i + 6)) {
                    return Go (Sr, i + 6);
                } else if (EqualSubs ("remaerd", Sr, i, i + 7)) {
                    return Go (Sr, i + 7);
                } else {
                    return false;
                }
        ...
        public static bool EqualSubs (string s1, string s2, int s2begin, int s2end) {
            if (s2.Length < s2end) {
                return false;
            } else {
                return Enumerable.Range (s2begin, s2end - s2begin)
                    .Select (i => s2[i])
                    .SequenceEqual (s1);
            }
        }
