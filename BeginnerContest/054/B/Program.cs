...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var N = NM[0];
            var M = NM[1];
            var A = Enumerable.Range (0, N).Select (i => Read ().ToList ()).ToList ();
            var B = Enumerable.Range (0, M).Select (i => Read ().ToList ()).ToList ();
            var isYes = Enumerable.Range (0, N - M + 1).Any (dy =>
                Enumerable.Range (0, N - M + 1).Any (dx => {
                    var isEqual = Enumerable.Range (0, M).All (i =>
                        Enumerable.Range (0, M).All (j =>
                            A[i + dy][j + dx] == B[i][j]));
                    return isEqual;
                }));
