...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var firsts = s[N - 1] == 'o' ? Enumerable.Range (1, 4) : Enumerable.Range (5, 4);
            var transitions = firsts.Select (first => s.Scanl (first, (accm, si) => Next (si == 'o', accm)));
            var Oks = transitions.Where (transition => transition.Last () == transition.First ());
            if (Oks.IsEmpty ()) {
                Print (-1);
            } else {
                var example = String.Concat (Oks.First ().Skip (1).Select (i => ToKind (i) ? "S" : "W"));
                Print (example);
            }
        }

        public static int RingIndex (int i, int n) {
            return (n + i) % n;
        }

        public static int Next (bool s, int perm) {
            if (s) {
                switch (perm) {
                    case 1:
                        return 1;
        ...
        public static bool ToKind (int trans) {
            switch (trans) {
                case 1:
                    return true;
                case 2:
