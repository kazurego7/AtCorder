...
namespace AtCoderTemplate {
    class Program {
        static void Main (string[] args) {
            var n = ReadInt ();
            var v = ReadInts ();
            var odd = Enumerable.Range (0, n).Where (i => (i + 1) % 2 == 1).Select (i => v[i]).ToList ();
            var oddGroups = odd.GroupBy (x => x).OrderByDescending (x => x.Count ());
            var even = Enumerable.Range (0, n).Where (i => (i + 1) % 2 == 0).Select (i => v[i]).ToList ();
            var evenGroups = even.GroupBy (x => x).OrderByDescending (x => x.Count ());

            var oddMaxNum = oddGroups.First ().First ();
            var evenMaxNum = evenGroups.First ().First ();
            var oddMaxCount = oddGroups.First ().Count ();
            var evenMaxCount = evenGroups.First ().Count ();
            var result = 0;
            if (oddMaxNum != evenMaxNum) {
                result = n - (oddMaxCount + evenMaxCount);
            } else {
                if (oddMaxCount == evenMaxCount) {
                    if (oddMaxCount == n / 2) {
                        result = n / 2;
                    } else {
                        var oddMaxCount2 = oddGroups.Skip (1).First ().Count ();
                        var evenMaxCount2 = evenGroups.Skip (1).First ().Count ();
                        if (oddMaxCount2 >= evenMaxCount2) {
                            result = n - (oddMaxCount2 + evenMaxCount);
                        } else {
                            result = n - (oddMaxCount + evenMaxCount2);
                        }
                    }
                } else {
                    var oddMaxCount2 = oddGroups.Skip (1).First ().Count ();
