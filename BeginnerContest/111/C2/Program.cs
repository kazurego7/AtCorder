...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var n = ReadInt ();
            var v = ReadInts ();
            var v1 = Interval (0, n).Where (i => IsEven (i)).Select (i => v[i]).GroupBy (x => x).OrderByDescending (g => g.Count ()).ToList ();
            var v2 = Interval (0, n).Where (i => IsOdd (i)).Select (i => v[i]).GroupBy (x => x).OrderByDescending (g => g.Count ()).ToList ();

            var ans = 1000000;
            if (v1[0].Key != v2[0].Key) {
                ans = n - (v1[0].Count () + v2[0].Count ());
            } else {
                if (v1.Count == 1 && v2.Count == 1) {
                    ...
                    if (v1.Count == 1) {
                        ...
                        ans = Min (n - (v1[0].Count () + v2[1].Count ()), n - (v1[1].Count () + v2[0].Count ()));
                    }
                }
            }
            Print (ans);
