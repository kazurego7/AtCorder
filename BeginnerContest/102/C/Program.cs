...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var B = Enumerable.Zip (A, Enumerable.Range (1, N), (a, i) => a - i).OrderBy (x => x).ToList ();
            if (isOdd (N)) {
                var sum = B.Select (b => Abs (b - B[N / 2])).Sum ();
                Print (sum);
            } else {
                var sum1 = B.Select (b => Abs (b - B[N / 2])).Sum ();
                var sum2 = B.Select (b => Abs (b - B[N / 2 + 1])).Sum ();
                Print (Min (sum1, sum2));
            }
        }
    }
