...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var A = ReadInts ();
            var orderA = A.OrderBy (x => x);
            var divisor = (int) Pow (10, 9) + 7;
            if (IsOdd (N)) {
                var OKs = Enumerable.Range (1, (N - 1) / 2).SelectMany (i =>
                    Enumerable.Repeat (i, 2).Select (m => 2 * m)
                ).Prepend (0);
                var isOK = orderA.SequenceEqual (OKs);
                var ans = PowRem (2, (N - 1) / 2, divisor);
                PrintIf (isOK, ans, 0);
            } else {
                var OKs = Enumerable.Range (1, N / 2).SelectMany (i =>
                    Enumerable.Repeat (i, 2).Select (m => 2 * m - 1)
                );
                var isOK = orderA.SequenceEqual (OKs);
                var ans = PowRem (2, N / 2, divisor);
                PrintIf (isOK, ans, 0);
            }
        }

        public static int PowRem (long x, int y, int divisor) {
