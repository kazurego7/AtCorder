...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var A = AB[0].ToInts ();
            var B = AB[1].ToInts ();
            var orderI = Enumerable.Range (0, N).OrderBy (i => B[i]).ToList ();
            var t = orderI.Scanl (0L, (accm, i) => accm + A[i]).Skip (1).ToList ();
            // PrintRow (t);
            // PrintRow (Enumerable.Range (0, N).Select (i => B[orderI[i]]));
            var isYes = Enumerable.Range (0, N).All (i => t[i] <= B[orderI[i]]);
