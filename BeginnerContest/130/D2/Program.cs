...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var K = NK[1].ToLong ();
            var a = ReadLongs ();
            var sumL = a.Scanl (0L, (accm, ai) => accm + ai).ToList ();
            var init = 0L;
            Func<int, int, bool> pred = (l, r) => sumL[r] - sumL[l] >= K;
            Func<int, int, long, long> update = (l, r, result) => result + (N + 1 - r);
            var ans = TwoPointersRightMethod (N + 1, pred, init, update);
            Print (ans);
        }
    }
