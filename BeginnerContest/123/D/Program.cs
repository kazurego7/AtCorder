...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var abc = new List<long> ();
            foreach (var k in Enumerable.Range (0, Min (K, X * Y))) {
                foreach (var z in Enumerable.Range (0, Z)) {
                    abc.Add (ab[k] + C[z]);
                }
            }
            abc.Sort ();
            abc.Reverse ();
            PrintColomn (abc.Take (K));
        }
    }
