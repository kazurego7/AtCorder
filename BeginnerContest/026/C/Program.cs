...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var B = ReadColumns (N - 1, 1) [0].ToInts ();
            var sub = Interval (0, N).Select (_ => new List<int> ()).ToList ();
            foreach (var i in Interval (0, N - 1)) {
                sub[B[i] - 1].Add (i + 1);
            }
            var p = new int[N];
            foreach (var i in Interval (0, N).Reverse ()) {
                if (sub[i].IsEmpty ()) {
                    p[i] = 1;
                } else {
                    var max = sub[i].Max (subi => p[subi]);
                    var min = sub[i].Min (subi => p[subi]);
                    p[i] = max + min + 1;
                }
            }
            // PrintRow (p);
