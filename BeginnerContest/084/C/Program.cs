...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var CSF = ReadIntColumns (N - 1);
            var C = CSF[0].Prepend (0).ToList ();
            var S = CSF[1].Prepend (0).ToList ();
            var F = CSF[2].Prepend (0).ToList ();

            var ans = Enumerable.Range (1, N).Select (k =>
                Enumerable.Range (k, N - k).Aggregate (0, (T, i) => {
                    if (T < S[i]) {
                        return S[i] + C[i];
                    } else if (T % F[i] == 0) {
                        return T + C[i];
                    } else {
                        return T + (F[i] - T % F[i]) + C[i];
                    }
                }));
            PrintColomn (ans);
        }
