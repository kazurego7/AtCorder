...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var wi0 = Enumerable.Range (0, N).Where (i => w[i] == w0).OrderByDescending (i => v[i]);
            var wi1 = Enumerable.Range (0, N).Where (i => w[i] == w1).OrderByDescending (i => v[i]);
            var wi2 = Enumerable.Range (0, N).Where (i => w[i] == w2).OrderByDescending (i => v[i]);
            var wi3 = Enumerable.Range (0, N).Where (i => w[i] == w3).OrderByDescending (i => v[i]);

            // 累積和
            var vS0 = wi0.Scanl (0L, (accm, wi) => accm + v[wi]).ToList ();
            var vS1 = wi1.Scanl (0L, (accm, wi) => accm + v[wi]).ToList ();
            var vS2 = wi2.Scanl (0L, (accm, wi) => accm + v[wi]).ToList ();
            var vS3 = wi3.Scanl (0L, (accm, wi) => accm + v[wi]).ToList ();

            var max = 0L;
            foreach (var x0 in Enumerable.Range (0, vS0.Count ())) {
                foreach (var x1 in Enumerable.Range (0, vS1.Count ())) {
                    foreach (var x2 in Enumerable.Range (0, vS2.Count ())) {
                        foreach (var x3 in Enumerable.Range (0, vS3.Count ())) {
                            if (x0 * w0 + x1 * w1 + x2 * w2 + x3 * w3 <= W) {
                                max = Max (max, vS0[x0] + vS1[x1] + vS2[x2] + vS3[x3]);
                            }
                        }
                    }
                }
