...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var max = Enumerable.Range (0, (int) Floor (F / (A * 100.0)) + 1).SelectMany (x =>
                    Enumerable.Range (0, (int) Floor ((F - 100 * A * x) / (B * 100.0)) + 1).SelectMany (y => {
                        var maxSolt = (A * x + B * y) * E;
                        // Print ($"{x} {y} {maxSolt}");
                        return Enumerable.Range (0, (int) Floor ((double) maxSolt / C) + 1).SelectMany (z =>
                            Enumerable.Range (0, (int) Floor ((double) (maxSolt - C * z) / D) + 1).Select (w => {
                                return new { water = 100 * (A * x + B * y), solt = C * z + D * w };
                            }));
                    }))
                .Where (sw => sw.water + sw.solt <= F)
                .OrderByDescending (sw => sw.solt / (double) sw.water);
            if (max.IsEmpty ()) {
                Print ($"0 0");
            } else {
                var ans = max.First ();
                Print ($"{ans.water + ans.solt} {ans.solt}");
            }
        }
    }
