...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var N = NM[0];
            var M = NM[1];
            var kss = new List<List<int>> ();
            foreach (var i in Enumerable.Range (0, M)) {
                kss.Add (ReadInts ());
            }
            var k = kss.Select (ks => ks.First ()).ToList ();
            var s = kss.Select (ks => ks.Skip (1).ToList ()).ToList ();
            var p = ReadInts ();

            var switchFlag = Enumerable.Range (0, (int) Pow (2, N));
            var switchDicts = new List<Dictionary<int, bool>> ();
            foreach (var flag in switchFlag) {
                var dict = new Dictionary<int, bool> ();
                foreach (var i in Enumerable.Range (0, N)) {
                    dict.Add (i + 1, (flag >> i) % 2 != 0);
                }
                switchDicts.Add (dict);
            }
            var ans = switchDicts.Where (dict =>
                    Enumerable.Range (0, M).All (i =>
                        s[i].Where (sij => dict[sij]).Count () % 2 == p[i]))
                // .Select (dict => {
                //     PrintRow (dict);
                //     return dict;
                // })
