...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var a = ab[0];
            var b = ab[1];
            var from1s = Enumerable.Range (0, M).Where (i => a[i] == 1).Select (i => b[i]);
            // PrintRow (from1s);
            var canGoToN = Enumerable.Range (0, N).Select (i => false).ToList ();
            foreach (var from in Enumerable.Range (0, M).Where (i => b[i] == N).Select (i => a[i])) {
                canGoToN[from - 1] = true;
            }
            // PrintRow (canGoToN);
            var isPossible = from1s.Any (middle => canGoToN[middle - 1]);
            PrintIf (isPossible, "POSSIBLE", "IMPOSSIBLE");
