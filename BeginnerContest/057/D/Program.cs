...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var decendingVs = v.OrderByDescending (x => x).ToList ();
            var maxAverage = (double) decendingVs.Take (A).Sum () / A;
            var X = Enumerable.Range (0, N).Where (i => decendingVs[i] == decendingVs[A - 1]).Count ();
            var Y = Enumerable.Range (0, A).Where (i => decendingVs[i] == decendingVs[A - 1]).Count ();
            var sumC = Enumerable.Range (A, (B - A + 1)).Select (i => C[X, i]).Sum ();
            var choose = (decendingVs[0] != decendingVs[A - 1]) ? C[X, Y] : sumC;
            Print (maxAverage);
            Print (choose);
        }
    }
