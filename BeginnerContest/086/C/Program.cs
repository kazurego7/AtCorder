...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var txy = ReadIntColumns (N);
            var t = txy[0].Prepend (0).ToList ();
            var x = txy[1].Prepend (0).ToList ();
            var y = txy[2].Prepend (0).ToList ();

            var isYes = Enumerable.Range (1, N).All (i => {
                var dx = Abs (x[i] - x[i - 1]);
                var dy = Abs (y[i] - y[i - 1]);
                var dt = t[i] - t[i - 1];
                var restTime = dt - (dx + dy);
                var canReached = restTime >= 0 && restTime % 2 == 0;
                //Print ($"{dx} {dy} {dt}");
                return canReached;
            });
