...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var rr = (long) (W % 3 != 0 ? 1 : 0) * H;
            var cc = (long) (H % 3 != 0 ? 1 : 0) * W;
            var rc = Interval (1, W).Select (w => {
                var rc1 = (long) (W - w) * (H / 2);
                var rc2 = (long) (W - w) * (H / 2 + H % 2);
                var rc3 = (long) w * H;
                return Max (rc1, Max (rc2, rc3)) - Min (rc1, Min (rc2, rc3));
            }).Min ();

            var cr = Interval (1, H).Select (h => {
                var cr1 = (long) (H - h) * (W / 2);
                var cr2 = (long) (H - h) * (W / 2 + W % 2);
                var cr3 = (long) h * W;
                return Max (cr1, Max (cr2, cr3)) - Min (cr1, Min (cr2, cr3));
            }).Min ();

            var ans = new List<long> { rr, cc, rc, cr }.Min ();
            Print (ans);
        }
    }
