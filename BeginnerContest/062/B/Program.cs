...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            // Print (a[0].Length);
            var ans = Enumerable.Range (0, H + 2).Select (h => Enumerable.Range (0, W + 2).Select (w => '#').ToList ()).ToList ();
            foreach (var h in Enumerable.Range (0, H + 2)) {
                foreach (var w in Enumerable.Range (0, W + 2)) {
                    if (h == 0 || h == H + 2 - 1 || w == 0 || w == W + 2 - 1) {
                        Console.Write ('#');
                    } else {
                        Console.Write (a[h - 1][w - 1]);
                    }
                }
                Console.Write ('\n');
            }
