...
namespace AtCoderTemplate
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            var SA = A.Scanl(0L, (x, y) => x + y).ToList();
            var SB = B.Scanl(0L, (x, y) => x + y).ToList();

            var ans = Interval(0, N + 1)
            .Select(i =>
            {
                var rest = K - SA[i];
                if (rest < 0)
                ...
                {
                    var count = BinarySearch(x => rest - SB[(int)x] >= 0L, M + 1, 0);
                    return count + i;
                }
            })
            .Max();
    ...
    public static class MyAlgorithm
    {
        ...
        public static long BinarySearch(Func<long, bool> predicate, long ng, long ok)
        {
            while (Abs(ok - ng) > 1)
            {
                var mid = (ok + ng) / 2L;
