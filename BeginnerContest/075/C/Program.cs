...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var edgeNums = Enumerable.Repeat (0, N).ToList ();
            ...
            PrintRow (edgeNums);
            var ans = Enumerable.Range (0, M).Where (m => {
                    var edgeNums2 = Enumerable.Range (0, N)
                        .Select (n => n == a[m] - 1 || n == b[m] - 1 ? edgeNums[n] - 1 : edgeNums[n]);
                    PrintRow (edgeNums2);
                    return edgeNums2.Where (n => n == 1).Count () > 2 || edgeNums2.Any (n => n == 0);
                })
                .Count ();
            Print (ans);
        }
