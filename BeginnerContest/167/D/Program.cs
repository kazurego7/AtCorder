...
namespace AtCoderTemplate
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            var B = A.Select(Ai => Ai - 1).ToList();

            var n = 0;
            var reached = Enumerable.Repeat(false, N).ToList();
            var node = 0;
            while (!reached[node])
            {
                n += 1;
            ...
            var m = 0;
            var reached2 = Enumerable.Repeat(false, N).ToList();
            var node2 = node;
            while (!reached2[node2])
            {
                m += 1;
                reached2[node2] = true;
                node2 = B[node2];
            ...
            var K2 = n - m >= K ? (int)K : (int)((K - (n - m)) % m + (n - m));
            // Print((n, m, K2));
            var next = 0;
            foreach (var _ in MyEnumerable.Interval(0, K2))
            {
                next = B[next];
            }
            Print(next + 1);
