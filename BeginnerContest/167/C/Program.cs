    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            foreach (var i in MyEnumerable.Interval(0, N))
            {
                var CA = ReadInts();
                C[i] = CA[0];
                foreach (var j in MyEnumerable.Interval(0, M))
                {
                    A[i, j] = CA[j + 1];
                }
            }

            var inf = (int)Pow(10, 7);

            var A0 = Enumerable.Repeat(0, M).ToArray();
            var q = new Queue<(int, bool, int, int[])>(new[] { (0, false, 0, A0), (0, true, 0, A0) });
            var ans = inf;

            while (q.Count != 0)
            {
                var (n, b, s, A1) = q.Dequeue();
                ...
                var s2 = s;
                var A2 = new List<int>(A1).ToArray();
                if (b)
                {
                    s2 += C[n];
                ...
                q.Enqueue((n + 1, false, s2, A2));
                q.Enqueue((n + 1, true, s2, A2));
            }

            PrintIf(ans != inf, ans, -1);
        }
    }
