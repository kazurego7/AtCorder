...
namespace AtCoderTemplate {
    class Program {
        static void Main (string[] args) {
            ...
            var sN = string.Concat (Group (s).Select (x => string.Concat (x.First (), x.Count ())));
            Console.WriteLine (sN);
        }

        static List<List<char>> Group (string source) {
            var group = new List<List<char>> ();
            group.Add (new List<char> { source[0] });
            foreach (var i in Enumerable.Range (1, source.Count () - 1)) {
                if (source[i] == source[i - 1]) {
                    group.Last ().Add (source[i]);
                } else {
                    group.Add (new List<char> { source[i] });
                }
            }
            return group;
        }
