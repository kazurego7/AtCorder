...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var A = ReadLongs ();
            var F = A.Scanl1 (GCD).ToList ();
            var G = Enumerable.Reverse (A).Scanl1 (GCD).Reverse ().ToList ();
            var ans = Enumerable.Range (1, N - 2).Select (i => GCD (F[i - 1], G[i + 1])).Append (F[N - 2]).Append (G[1]).Max ();
            Print (ans);
        }
    }
