...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var N = ND[0];
            var D = ND[1];
            var X = ReadRows (N).Select (row => row.ToInts ()).ToList ();

            var ans = MyEnumerable.Interval (0, N).SelectMany (i =>
                    MyEnumerable.Interval (i + 1, N)
                    .Select (j =>
                        Enumerable.Range (0, D).Select (d =>
                            Pow (X[i][d] - X[j][d], 2)
                        ).Sum ()
                    )
                ).Where (ds => Pow ((int) Sqrt (ds), 2) == ds)
                //.Do (ds => Print (ds))
                .Count ();
