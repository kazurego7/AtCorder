...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var B = ABQ[1];
            var Q = ABQ[2];
            var s = ReadColumns (A, 1) [0].ToLongs ();
            var t = ReadColumns (B, 1) [0].ToLongs ();
            var x = ReadColumns (Q, 1) [0].ToLongs ();

            var maxd = (long) Pow (10, 11);

            var ans = Enumerable.Range (0, Q)
                .Select (i => {
                    Func<long, bool> over = (y => y >= x[i]);
                    Func<long, bool> under = (y => y < x[i]);
                    var ioA = RightBinarySearch (s, over);
                    var iuA = LeftBinarySearch (s, under);
                    var ioB = RightBinarySearch (t, over);
                    var iuB = LeftBinarySearch (t, under);
                    ...
                    var oAoB = ex_ioA && ex_ioB ?
                        Min (Abs (x[i] - s[ioA]), Abs (x[i] - t[ioB])) + Abs (s[ioA] - t[ioB]) :
                        maxd;
                    var uAoB = ex_iuA && ex_ioB ?
                        Min (Abs (x[i] - s[iuA]), Abs (x[i] - t[ioB])) + Abs (s[iuA] - t[ioB]) :
                        maxd;
                    var oAuB = ex_ioA && ex_iuB ?
                        Min (Abs (x[i] - s[ioA]), Abs (x[i] - t[iuB])) + Abs (s[ioA] - t[iuB]) :
                        maxd;
                    var uAuB = ex_iuA && ex_iuB ?
                        Min (Abs (x[i] - s[iuA]), Abs (x[i] - t[iuB])) + Abs (s[iuA] - t[iuB]) :
                        maxd;

                    return new List<long> { oAoB, uAoB, oAuB, uAuB }.Min ();
                });
            PrintColomn (ans);
        }
    }
