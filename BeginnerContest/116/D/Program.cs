...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var N = NK[0];
            var K = NK[1];
            var td = ReadColumns (N, 2);
            var t = td[0].ToInts ();
            var d = td[1].ToInts ();

            var sorted = Interval (0, N).OrderByDescending (i => d[i]);

            var G1 = sorted.Take (K).GroupBy (i => t[i]);
            var S = new PriorityQueue<int> ((s1, s2) => d[s1] - d[s2]);
            foreach (var i in G1.Where (gg => gg.Count () != 1).SelectMany (gg => gg.OrderBy (i => d[i]).Take (gg.Count () - 1))) {
                // Print (i);
                S.Enqueue (i);
            }
            var selected = new bool[N + 1];
            foreach (var i in G1.Select (gg => gg.Key)) {
                selected[i] = true;
            }
            var G2 = sorted.Skip (K).Where (i => !selected[t[i]]).GroupBy (i => t[i]);
            var nonS = new PriorityQueue<int> ((s1, s2) => -(d[s1] - d[s2]));
            foreach (var s in G2.Select (gg => gg.OrderByDescending (i => d[i]).First ())) {
                nonS.Enqueue (s);
            }

            // Print ("#");
            var f = Enumerable.Repeat (-1L, N + 1).ToList ();
            var x = G1.Count ();
            f[x] = G1.SelectMany (gg => gg.Select (gi => (long) d[gi])).Sum ();
            foreach (var a in x + 1 <= N + 1 ? Interval (x + 1, N + 1) : Enumerable.Empty<int> ()) {
                if (S.Size () == 0 || nonS.Size () == 0) break;
                // Print ($"{d[S.Peek ()]} {d[nonS.Peek ()]}");
                var minS = S.Dequeue ();
                var maxNonS = nonS.Dequeue ();
                f[a] = f[a - 1] - d[minS] + d[maxNonS];
            }

            var g = Enumerable.Repeat (-1L, N + 1).ToList ();
            foreach (var i in Interval (1, N + 1)) {
                if (f[i] == -1) continue;
                g[i] = f[i] + (long) i * i;
            }
            // Print ("#");

            // foreach (var g1 in G1) {
            //     PrintRow (g1.Select (i => d[i]));
            // }
            // Print ("#");
            // foreach (var g2 in G2) {
            //     PrintRow (g2.Select (i => d[i]));
            // }
            // // Print ($"{sorted.GroupBy (gi => t[gi]).Count ()} {G1.Count()} {G2.Count()}");
            // PrintRow (f);
            // PrintRow (g);
            var ans = g.Max ();
            Print (ans);
