...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var X = ReadInt ();
            var p2max = 31;
            var bp2s = Enumerable.Range (1, p2max).SelectMany (b => Enumerable.Range (2, 10).Select (p => (int) Pow (b, p)).Where (bp => bp <= 1000)).OrderByDescending (x => x);
            var perfectPower = bp2s.First (pow => pow <= X);
            Print (perfectPower);
        }
    }
