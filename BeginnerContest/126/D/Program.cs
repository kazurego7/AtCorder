...
namespace AtCoderTemplate {
    public class Program {
        ...
        public static void Main (string[] args) {
            ...
            var uvw = ReadLongColumns (N - 1);
            var u = uvw[0].Select (i => (int) i - 1).ToList ();
            var v = uvw[1].Select (i => (int) i - 1).ToList ();
            var w = uvw[2];

            var dict = new Dictionary<int, color> ();
            foreach (var i in Enumerable.Range (0, N)) {
                dict.Add (i, color.none);
            }

            if (IsEven (w[0])) {
                dict[u[0]] = color.black;
                dict[v[0]] = color.black;
            } else {
                dict[u[0]] = color.white;
                dict[v[0]] = color.white;
            }

            var queue = new Queue<int> (Enumerable.Range (1, N - 2));

            while (!queue.IsEmpty ()) {
                var i = queue.Dequeue ();
                if (dict[u[i]] != color.none && dict[v[i]] == color.none) {
                    if (IsEven (w[i])) {
                        dict[v[i]] = dict[u[i]];
                    } else {
                        dict[v[i]] = Reverse (dict[u[i]]);
                    }
                } else if (dict[v[i]] != color.none && dict[u[i]] == color.none) {
                    if (IsEven (w[i])) {
                        dict[u[i]] = dict[v[i]];
                    } else {
                        dict[u[i]] = Reverse (dict[v[i]]);
                    }
                } else {
                    queue.Enqueue (i);
                }
            ...
            foreach (var d in dict) {
                if (d.Value == color.white) {
                    Print ("0");
                } else if (d.Value == color.black) {
                    Print ("1");
                } else {
                    Print ("none");
                }
        ...
        public static color Reverse (color x) {
            if (x == color.white) {
                return color.black;
            } else {
                return color.white;
