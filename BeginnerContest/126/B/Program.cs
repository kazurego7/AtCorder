...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var front = S.Substring (0, 2);
            var back = S.Substring (2, 2);
            if (IsMM (front) && IsMM (back)) {
                Print ("AMBIGUOUS");
            } else if (IsMM (front) && !IsMM (back)) {
                Print ("MMYY");
            } else if (!IsMM (front) && IsMM (back)) {
                Print ("YYMM");
            } else {
        ...
        public static bool IsMM (string str) {
            if (str[0] == '0') {
                return str[1] != '0';
            }
            if (str[0] == '1') {
                return str[1] == '0' || str[1] == '1' || str[1] == '2';
            } else {
                return false;
            }
        }
