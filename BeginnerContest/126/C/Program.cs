...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var log2KCount = (int) Log (K, 2) + 2;
            var pows = Enumerable.Range (0, log2KCount).Select (i => Pow (2, i)).ToList ();
            // PrintRow (pows);
            var ps = Enumerable.Range (1, N).Select (n => {
                var overK = Enumerable.Range (0, log2KCount).First (r => n * pows[r] >= K);
                var p = (double) 1 / (N * pows[overK]);
                return p;
            });
            Print (ps.Sum ());
        }
