...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var K = NKQ[1];
            var Q = NKQ[2];
            var A = ReadColumns (Q, 1) [0].ToInts ();

            var correctCounts = new int[N];
            foreach (var g in A.GroupBy (g => g)) {
                correctCounts[g.Key - 1] = g.Count ();
            }

            var ans = Interval (0, N).Select (i => K - Q + correctCounts[i] > 0 ? "Yes" : "No");
            PrintColomn (ans);
        }
    }
