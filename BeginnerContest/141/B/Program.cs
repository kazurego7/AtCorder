...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var S = Read ();
            var odd = Interval (0, S.Length).Where (i => IsEven (i)).All (i => S[i] == 'R' || S[i] == 'U' || S[i] == 'D');
            var even = Interval (0, S.Length).Where (i => IsOdd (i)).All (i => S[i] == 'L' || S[i] == 'U' || S[i] == 'D');
            PrintIf (odd && even, "Yes", "No");
        }
    }
