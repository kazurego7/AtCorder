...
namespace AtCoderTemplate
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            var M = NMQ[1];
            var Q = NMQ[2];
            var abcd = ReadColumns(Q, 4);
            var a = abcd[0].ToInts();
            var b = abcd[1].ToInts();
            var c = abcd[2].ToInts();
            var d = abcd[3].ToLongs();

            var ans = 0L;

            void dfs(IEnumerable<int> seq)
            {
                if (seq.Count() == N)
                {
                    var A = seq.ToList();
                    var ans0 = MyEnumerable.Interval(0, Q)
                    .Select(i => A[b[i] - 1] - A[a[i] - 1] == c[i] ? d[i] : 0L)
                    .Sum();
                    ans = Max(ans, ans0);
                    return;
                }
                ...
                var m = seq.Last();
                foreach (var i in MyEnumerable.Interval(m, M + 1))
                {
                    dfs(seq.Append(i));
                }
            }
