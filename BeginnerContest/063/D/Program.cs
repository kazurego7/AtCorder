...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var A = NAB[1];
            var B = NAB[2];
            var h = ReadLongColumns (N) [0];

            var ng = -1L;
            var ok = h.Max () / B + (h.Max () % B != 0 ? 1 : 0);
            while (Abs (ng - ok) > 1) {
                var mid = (ng + ok) / 2;
                if (Enough (A, B, h, mid)) {
                    ok = mid;
        ...
        static bool Enough (long A, long B, IEnumerable<long> h, long T) {
            var count = h.Select (hi => hi - T * B)
                .Where (damagedH => damagedH > 0)
                .Select (damagedH => damagedH / (A - B) + (damagedH % (A - B) != 0 ? 1 : 0))
                .Sum ();
            return count <= T;
        }
    }
