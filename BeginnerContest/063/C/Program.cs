...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var s = ReadIntColumns (N) [0];

            var mult10Sum = s.Where (si => si % 10 == 0).Sum ();
            var notMult10 = s.Where (si => si % 10 != 0);
            var notMult10Sum = notMult10.Sum ();

            if (notMult10.Count () == 0) {
                Print (0);
            } else if (notMult10Sum % 10 != 0) {
                Print (mult10Sum + notMult10Sum);
            } else {
                var notMult10Min = notMult10.Min ();
                Print (mult10Sum + notMult10Sum - notMult10Min);
            }
