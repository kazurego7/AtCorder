...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var N = NM[0];
            var M = NM[1];
            var quot = (long) (Pow (10, 9) + 7);
            if (Abs (N - M) == 0) {
                Print ((2 * BigFact (N) * BigFact (M)) % quot);
            } else if (Abs (N - M) == 1) {
                Print ((BigFact (N) * BigFact (M)) % quot);
            } else {
                Print (0);
            }
        }

        public static long BigFact (int n) {
            var quot = (long) (Pow (10, 9) + 7);
            // return Enumerable.Range (1, n).Aggregate (new BigInteger (1), (a, b) => BigInteger.Multiply (a, b));
            return Enumerable.Range (1, n).Aggregate ((long) 1, (a, b) => (a * b) % quot);
        }
    }
