...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            /*
            考え方
            まず、前提として10進数から2進数への変換を知っている必要があるので、その方法を説明する。
            26  = 2 * 13 + 0
                = 2 (2 * 6 + 1) + 0
                = 2 (2 (2 * 3 + 0) + 1) + 0
                = 2 (2 (2 (2 * 1 + 1) + 0) + 1) + 0     <- 展開
                
                = (2^4) * 1 + (2^3) * 1 + (2^2) * 0 + (2^1) * 1 + (2^0) * 0     <- 展開時の各項の2^nとあまりの積を利用する

            上の図から、2で割ったあまり が2進数の各桁になることがわかる。
            参考 (https://sci-pursuit.com/math/binary-number-conversion.html#2)

            10進数から(-2)進数への変換も、10進数から2進数への変換と同様の操作を行う。
            -9  = (-2) * 5 + 1
                = (-2) {(-2) * 2 + 1} + 1
                = (-2) {(-2) {(-2) * 1 + 0} + 1} + 1
            ...
            (*注意!)このとき、間違ってあまりが負の数ならないように気を付ける。
            以下は間違ってあまりが負の数になった例。
            -9  = (-2) * 4 + (-1)
                = (-2) * {(-2) * 2 + 0} + (-1)
                = (-2) * {(-2) * {(-2) * (-1) + 0} + 0} + (-1)

                = {(-2)^3} * (-1) + {(-2)^2} * 0 + {(-2)^1} * 0 + {(-2)^0} * (-1)  <- 展開しても、2進数への応用に使えない

            また、
            a = (-2)*b + r (0 < b)
            について、(a < 0)のとき、商bを求めるためには、
            r = a % (-2)
            として先にあまりrを求めてしまってから商bを求めると良い。
            (a > 0)についても同様の計算で行える。
        ...
        public static string Go (long quot, string ans) {
            if (quot == 0) {
                return ans + "0";
            } else if (quot == 1) {
                return ans + "1";
                ...
                var rest = Abs (quot) % 2;
                var quot2 = (quot - rest) / (-2);
                if (rest == 0) {
                    return Go (quot2, ans) + "0";
                } else {
                    return Go (quot2, ans) + "1";
                }
            }
        }
    }
