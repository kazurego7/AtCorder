...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var M = NM[1];
            var A = ReadLongs ();
            var BC = ReadColumns (M);
            var B = BC[0].ToInts ();
            var C = BC[1].ToLongs ();
            var descA = A.OrderByDescending (x => x).ToList ();
            var descI = Enumerable.Range (0, M).OrderByDescending (i => C[i]).ToList ();
            ...
            var max = 0L;
            while (true) {
                var aValue = aI < N ? descA[aI] : -1;
                var cValue = cI < M ? C[descI[cI]] : -1;

                // Print ($"{aI} {cI} {allCount} {max}");
                var count = 0;
                var value = 0L;
                if (aValue <= cValue) {
                    value = cValue;
                ...
                if (N - (allCount + count) <= 0) {
                    max += (N - allCount) * value;
                    break;
                } else {
                    allCount += count;
                    max += count * value;
