...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var s = Enumerable.Range (0, N).Select (_ => Console.ReadLine ()).ToList ();
            var M = ReadInt ();
            var t = Enumerable.Range (0, M).Select (_ => Console.ReadLine ()).ToList ();
            var res = s.Select (s1 =>
                s.Where (s2 => s1 == s2).Count () - t.Where (t2 => s1 == t2).Count ()
            ).Max ();
            Print (Max (res, 0));
