...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var azCounters = new List<List<int>> ();
            foreach (var i in Enumerable.Range (0, n)) {
                var azDict = new Dictionary<char, int> ();
                foreach (var c in Enumerable.Range ('a', 26).Select (k => (char) k)) {
                    azDict.Add (c, 0);
                }
                foreach (var c in S[i]) {
                    azDict[c] += 1;
                }
                azCounters.Add (azDict.Values.ToList ());
            }

            var azCountMin = azCounters.Aggregate ((az1, az2) => Enumerable.Range (0, 26).Select (i => Min (az1[i], az2[i])).ToList ());
            var ans = String.Concat (Enumerable.Range (0, 26).SelectMany (i => Enumerable.Repeat ('a' + i, azCountMin[i]).Select (k => (char) k)));
