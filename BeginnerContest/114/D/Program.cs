...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var e = new int[N + 1];
            foreach (var i in Interval (2, N + 1)) {
                var p = i;
                var j = 2;
                while (p != 1) {
                    if (p % j == 0) {
            ...
            Func<int, int> n = m => e.Count (ei => ei >= m - 1);
            var p75 = n (75);
            var p25_3 = n (25) * (n (3) - 1);
            var p15_5 = n (15) * (n (5) - 1);
            var p5_5_3 = (n (5) * (n (5) - 1)) / 2 * (n (3) - 2); // p^5 * q^5の組み合わせの重複を除くために / 2 
            var ans = p75 + p25_3 + p15_5 + p5_5_3;
            Print (ans);
        }
    }
