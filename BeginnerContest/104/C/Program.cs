...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var D = DG[0];
            var G = DG[1];
            var pc = ReadIntColumns (D);
            var p = pc[0];
            var c = pc[1];

            var result = Enumerable.Range (0, (int) Pow (2, D))
                .Select (x => {
                    var compIndex = Enumerable.Range (0, D).Where (di => (x >> di) % 2 == 1);
                    var noCompIndex = Enumerable.Range (0, D).Where (di => (x >> di) % 2 == 0);
                    var compScore = compIndex.Select (i => p[i] * (i + 1) * 100 + c[i]).Sum ();
                    var restScore = G - compScore;
                    var compCount = compIndex.Select (i => p[i]).Sum ();

                    if (restScore <= 0 || noCompIndex.IsEmpty ()) {
                        return compCount;
                    } else {
                        var maxNoCompI = noCompIndex.OrderByDescending (i => p[i]).First ();
                        var noCompMaxSums = Enumerable.Range (0, p[maxNoCompI]).Select (i => i * (maxNoCompI + 1) * 100);
                        if (restScore <= noCompMaxSums.Last ()) {
                            var noCompCount = noCompMaxSums.TakeWhile (sum => sum < restScore).Count ();
                            return compCount + noCompCount;
                        } else {
                            return int.MaxValue;
                        }
                    }
