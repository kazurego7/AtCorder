...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var a = ReadLongs ();
            var b = new long[n];
            var odds = Enumerable.Range (1, n).Where (isOdd).Select (i => a[i - 1]);
            var evens = Enumerable.Range (1, n).Where (isEven).Select (i => a[i - 1]);
            if (isOdd (n)) {
                PrintRow (odds.Reverse ().Concat (evens));
            } else {
                PrintRow (evens.Reverse ().Concat (odds));
            }
        }
    }
