...
namespace AtCoderTemplate {
    class Program {
        static void Main (string[] args) {
            ...
            (x2, y2)に対する(x1, y1)の相対座標を(absx1, absy1)とする。(つまり、(x2, y2)が原点だとしたときの(x1, y1))
            absx1 = x1 - x2
            absy1 = y1 - y2

            また、(x2, y2)に対する(x3, y3)の相対座標を(absx3, absy3)とする。
            absx3 = x3 - x2
            absy3 = y3 - y2

            このとき、(absx1, absy1)に対して(absx3, absy3)は、(x2, y2)を中心として-90度回転した点だと考えることができる。
            ...
            absx3 = absy1  <=> x3 - x2 = y1 - y2     <=> x3 = x2 + y1 - y2
            absy3 = -absx1 <=> y3 - y2 = - (x1 - x2) <=> y3 = y2 - x1 + x2 

            (x4, y4)についても、(x3,y3)に対する(x2, y2)と(x4, y4)の相対座標を求め、同様の計算をすればよい。
             */
