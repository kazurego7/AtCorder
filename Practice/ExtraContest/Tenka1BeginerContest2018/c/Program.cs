...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var A = ReadLongColumns (N) [0];

            /* 
            考え方
            以下の2つのパターンの内、最大のものが答えになる。

            パターン1
            Aを昇順にソートする => B
            ... B(N-3) B2 BN B1 B(N-1) B3 B(N-4) ...
            のように、最小値B1を中心として、最大値BN、B(N-1)ではさみ、また、最小値B2, B3ではさみ、
            と繰り返し最後に差分の和をとる。

            パターン2
            Aを降順にソートする => C
            ... C(N-3) C2 CN C1 C(N-1) C3 C(N-4) ...
            のように、最大値B1を中心として、最小値CN、C(N-1)ではさみ、また、最大値C2, C3ではさみ、
            と繰り返し最後に差分の和をとる。
            */

            var middleMin = new LinkedList<long> ();
            GoLtoR (new LinkedList<long> (A.OrderBy (x => x)), middleMin);

            var middleMax = new LinkedList<long> ();
            GoLtoR (new LinkedList<long> (A.OrderByDescending (x => x)), middleMax);

            var ans1 = middleMin.MapAdjacent ((a, b) => Abs (a - b)).Sum ();
            var ans2 = middleMax.MapAdjacent ((a, b) => Abs (a - b)).Sum ();
            Print (Max (ans1, ans2));
        }

        public static void GoRtoL (LinkedList<long> X, LinkedList<long> Res) {
            if (X.IsEmpty ()) {
                return;
            } else {
                Res.AddFirst (X.Last.Value);
                X.RemoveLast ();
                GoRtoR (X, Res);
                return;
            }
        }
        public static void GoRtoR (LinkedList<long> X, LinkedList<long> Res) {
            if (X.IsEmpty ()) {
                return;
            } else {
                Res.AddLast (X.Last.Value);
                X.RemoveLast ();
                GoLtoL (X, Res);
                return;
            }
        }
        public static void GoLtoL (LinkedList<long> X, LinkedList<long> Res) {
            if (X.IsEmpty ()) {
                return;
            } else {
                Res.AddFirst (X.First.Value);
                X.RemoveFirst ();
                GoLtoR (X, Res);
                return;
            }
        ...
        public static void GoLtoR (LinkedList<long> X, LinkedList<long> Res) {
            if (X.IsEmpty ()) {
                return;
            } else {
                Res.AddLast (X.First.Value);
                X.RemoveFirst ();
                GoRtoL (X, Res);
                return;
            }
