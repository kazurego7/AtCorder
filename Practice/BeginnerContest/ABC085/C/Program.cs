...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var xyzs = Enumerable.Range (0, x + 1)
                .SelectMany (xi => Enumerable.Range (0, y + x * 2 + 1)
                    .Where (yi => N - xi - yi >= 0 && xi * 10000 + yi * 5000 + (N - xi - yi) * 1000 == Y)
                    .Select (yi => new List<int> { xi, yi, N - xi - yi })
                );
            if (xyzs.IsEmpty ()) {
                Print ("-1 -1 -1");
            } else {
