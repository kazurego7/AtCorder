...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var cond1s = Enumerable.Range (0, S2d - Td + 1)
                .Where (i => IsOk (S2.Substring (i, Td), T, Td))
                .Select (i => String.Concat (S2.TakeAwayRange (i, Td).InsertEnumAt (i, T)));
            if (cond1s.IsEmpty ()) {
                Print ("UNRESTORABLE");
                return;
            }
            // PrintRow (cond1s);
            var cond2 = cond1s
                .Select (cond1 => cond1.Replace ('?', 'a'))
                .OrderBy (x => x)
                .First ();
            Print (cond2);
        }

        public static bool IsOk (string str1, string str2, int length) {
            return Enumerable.Range (0, length).All (i => str1[i] == '?' || str1[i] == str2[i]);
        }
    }
