...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var R = NMR[2];
            var r = ReadInts ();
            var ABC = Enumerable.Range (0, M).Select (_ => ReadInts ()).ToList ();
            var edgeGraph = new long[N, N];
            foreach (var i in Enumerable.Range (0, N)) {
                foreach (var j in Enumerable.Range (0, N)) {
                    if (i == j) {
                        edgeGraph[i, j] = 0;
                    } else {
                        edgeGraph[i, j] = 200 * 100000 + 1;
                    }
                }
            }
            ...
            var minDistances = edgeGraph;
            foreach (var b in Enumerable.Range (0, N)) {
                foreach (var a in Enumerable.Range (0, N)) {
                    foreach (var c in Enumerable.Range (0, N)) {
                        minDistances[a, c] = Min (minDistances[a, c], minDistances[a, b] + minDistances[b, c]);
                    }
                }
            }

            // Print2DArray (r.Perm (R));
            var ans = r.Perm (R)
                .Select (perm => {
                    var dists = perm.MapAdjacent ((a, b) => minDistances[a - 1, b - 1]);
                    // PrintRow (dists);
                    return dists.Sum ();
                })
                .Min ();
