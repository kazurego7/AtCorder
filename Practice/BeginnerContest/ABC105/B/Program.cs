...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            /*
            offset と count の変換が繰り返されるので、off-by-one-errorが非常に起きやすい
            計算は全てoffsetで行うことで、軽減できる。
            また、offsetに直すことで、逆順からの取り出しも容易になる。
             */
            var offsetR2 = (S.Count () - 1) - 1;
            var offsetL3R2 = offsetR2 - (3 - 1);
            var condC = Enumerable.Range (2, offsetL3R2 + 1).Where (i => S[i] == 'C').Count () == 1;

            var condLower = S.Count () - S.Where (char.IsLower).Count () == 2;
            PrintIf (condA && condC && condLower, "AC", "WA");
            // Print ($"{condA} {condC} {condLower}");
        }
    }
