...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var M = S.Where (Si => Si[0] == 'M').Count ();
            var A = S.Where (Si => Si[0] == 'A').Count ();
            var R = S.Where (Si => Si[0] == 'R').Count ();
            var C = S.Where (Si => Si[0] == 'C').Count ();
            var H = S.Where (Si => Si[0] == 'H').Count ();

            var not0 = new List<long> { M, A, R, C, H }.Where (i => i != 0).ToList ();

            if (not0.Count < 3) {
                Print (0);
            } else {
                var lastIx = not0.Count - 1;
                var ans = Enumerable.Range (0, lastIx - 2 + 1)
                    .SelectMany (a => Enumerable.Range (a + 1, lastIx - 1 - (a + 1) + 1)
                        .SelectMany (b => Enumerable.Range (b + 1, lastIx - (b + 1) + 1)
                            .Select (c => {
                                // Print ($"{a}, {b}, {c}");
                                return not0[a] * not0[b] * not0[c];
                            })))
                    .Sum ();
                Print (ans);
            }
