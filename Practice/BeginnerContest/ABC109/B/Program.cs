...
namespace AtCoderTemplate {
    class Program {
        static void Main (string[] args) {
            ...
            var WFirst = W.Select (c => c.First ()).ToList ();
            var WLast = W.Select (c => c.Last ()).ToList ();
            var isNotDup = W.Distinct ().Count () == W.Count ();
            var existNextAll = Enumerable.Range (0, WFirst.Count - 1).All (i => WFirst[i + 1] == WLast[i]);
            PrintIf (isNotDup && existNextAll, "Yes", "No");
        }
    }
