...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var pow6 = Enumerable.Range (1, 6).Select (i => (int) Pow (6, i));
            var pow9 = Enumerable.Range (1, 5).Select (i => (int) Pow (9, i));
            var subs = pow6.Concat (pow9).Append (1).OrderBy (x => x);
            var dp = new int[N + 1];
            dp[0] = 0;
            foreach (var i in Enumerable.Range (1, N)) {
                var conds = subs.TakeWhile (sub => sub <= i);
                var min = conds.Select (cond => dp[i - cond]).Min ();
                dp[i] = min + 1;
            }
