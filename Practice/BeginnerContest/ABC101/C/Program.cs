...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var i = A.IndexOf (1);
            var lrs = Enumerable.Range (0, K).Select (k => new List<int> { i - k, (i + K - 1) - k }).Where (lr => 0 <= lr[0] && lr[1] < N);

            var min = lrs.Select (lr => {

                var l = lr[0];
                var r = lr[1];
                // Print ($"{l} {r}");
                var countL = l == 0 ? 0 : Ceiling ((double) l / (K - 1));
                var countR = r == N ? 0 : Ceiling ((double) (N - 1 - r) / (K - 1));
