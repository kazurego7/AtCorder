...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var A = ReadInts ().OrderBy (x => x).ToList ();
            var B = ReadInts ();
            var C = ReadInts ().OrderBy (x => x).ToList ();

            var ans = B.Select (b => {
                var countA = (BinarySearch ((i) => A[i] < b, A.Count, -1) + 1);
                var countC = (C.Count - BinarySearch ((i) => b < C[i], -1, C.Count));
                return (long) countA * countC;
            }).Sum ();
            Print (ans);
        }

        public static int BinarySearch (Func<int, bool> solve, int ng, int ok) {
            while (Abs (ok - ng) > 1) {
                int mid = (ok + ng) / 2;
                if (solve (mid)) {
                    ok = mid;
