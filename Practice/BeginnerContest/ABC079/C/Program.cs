...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var ABCD = Console.ReadLine ().Select (c => int.Parse (c.ToString ())).ToList ();
            var A = ABCD[0];
            var B = ABCD[1];
            var C = ABCD[2];
            var D = ABCD[3];
            var plusMinus = new List<string> { "+", "-" };
            var ans = plusMinus
                .SelectMany (op1 => plusMinus
                    .SelectMany (op2 => plusMinus
                        .Where (op3 => calc (calc (calc (A, B, op1), C, op2), D, op3) == 7)
                        .Select (op3 => $"{A}{op1}{B}{op2}{C}{op3}{D}=7")));
            Print (ans.First ());
