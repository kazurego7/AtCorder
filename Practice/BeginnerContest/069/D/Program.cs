...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var N = ReadInt ();
            var a = ReadInts ();
            var list = Enumerable.Range (1, N).SelectMany (i => Enumerable.Repeat (i, a[i - 1])).ToList ();
            var snakeGrid = Enumerable.Range (0, H).Select (h => {
                if (isEven (h)) {
                    return Enumerable.Range (0, W).Select (w => list[h * W + w]);
                } else {
                    return Enumerable.Range (0, W).Reverse ().Select (w => list[h * W + w]);
                }
            });
            Print2DArray (snakeGrid);
        }
    }
