...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            var N = ReadInt ();
            var a = ReadLongs ();
            var group = a.GroupBy (x => x).OrderBy (x => x.Count ()).ToList ();
            var b = group.Select (x => x.First ()).ToList ();
            if (N % 3 != 0) {
                Print ("No");
            } else if (group.Count () == 3) {
                PrintIf ((b[0] ^ b[1]) == b[2] && group.All (x => x.Count () == N / 3), "Yes", "No");
            } else if (group.Count () == 2) {
                PrintIf (b[0] == 0 && (group[0].Count () == N / 3 && group[1].Count () == 2 * N / 3), "Yes", "No");
            } else if (group.Count () == 1) {
                PrintIf (b[0] == 0, "Yes", "No");
            } else {
                Print ("No");
            }
        }
