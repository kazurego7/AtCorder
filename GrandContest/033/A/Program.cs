...
namespace AtCoderTemplate {
    public class Program {
        public static void Main (string[] args) {
            ...
            var sharps = Enumerable.Range (0, H).SelectMany (h => Enumerable.Range (0, W).Where (w => A[h][w] == '#').Select (w => new List<int> { h, w })).ToList ();
            Print (Go (new Queue<List<int>> (sharps), H, W, A, 0));
        }

        public static int Go (Queue<List<int>> queue, int H, int W, List<List<char>> A, int count) {

            var next = new Queue<List<int>> ();
            while (!queue.IsEmpty ()) {
                ...
                if (0 <= x - 1 && A[y][x - 1] == '.') {
                    A[y][x - 1] = '#';
                    next.Enqueue (new List<int> { y, x - 1 });
                }
                if (x + 1 < W && A[y][x + 1] == '.') {
                    A[y][x + 1] = '#';
                    next.Enqueue (new List<int> { y, x + 1 });
                ...
                if (0 <= y - 1 && A[y - 1][x] == '.') {
                    A[y - 1][x] = '#';
                    next.Enqueue (new List<int> { y - 1, x });
                }
                if (y + 1 < H && A[y + 1][x] == '.') {
                    A[y + 1][x] = '#';
                    next.Enqueue (new List<int> { y + 1, x });
